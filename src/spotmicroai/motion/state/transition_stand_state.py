...
class TransitionStandState(BaseState):
    """Transition from idle/lying to standing position."""
    ...
    def handle_input_commands(
        ...
        """Handle input commands during transition to stand."""
        ...
        if self.check_body_state_equality(body_state, self.end_body_state_, 0.001):
            from spotmicroai.motion.state.stand_state import StandState

            self.change_state(motion_controller, StandState())
    ...
    def init(self, body_state: BodyState, command: Command, motion_controller: "MotionController") -> None:
        """Initialize transition to stand state."""
        ...
        self.end_body_state_.xyz_positions.y = CONSTANTS.DEFAULT_STAND_HEIGHT
